...
export const LessonVideo:React.FC<LessonProps>=(props)=>{const {fps}=useVideoConfig();let cursor=0;const output:React.ReactNode[]=[];
  if(props.openingVideo){const df=frames(props.openingVideo.durationSeconds,fps);output.push(<Sequence key="linked-opening" from={cursor} durationInFrames={df}><LinkedVideoView video={props.openingVideo}/></Sequence>);cursor+=df;}
